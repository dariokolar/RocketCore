	...
	init : function( editor ) {
		...
		editor.on( 'simpleuploads.startUpload' , function(ev) {
			...
			if (data.mode.type=="base64paste")
			{
				...
				data.result = "<span id='" + idTmp + "' class='SimpleUploadsTmpWrapper'>&nbsp</span>";
				data.mode.id = idTmp;
			}
...
CKEDITOR.plugins.simpleuploads = {
	...
	insertProcessedFile: function(editor, evData) {
		...
		{
			case 'selectedFile':
				this.insertSelectedFile( editor, evData );
				break;
			...
			case 'callback':
				this.processFileWithCallback( editor, evData );
				break;
...
function createSVGAnimation( file, id, editor )
{
	...
	function createSimpleUpload( editor, dialogName, definition, element )
	{
		...
		/* tady puvodne byla nejaka logika, ktera se rada rozsypala,takhl natvrdo to zda se funguje. */
		var targetField = "info:url";
